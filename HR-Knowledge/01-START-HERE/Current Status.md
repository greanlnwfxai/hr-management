...
| 4 | RBAC | MANAGER approve/reject is department-scoped (v1.2.0) with a self-approval/self-rejection guard added in `HOTFIX-T089A`; `GET /leave` list access is now department-scoped for MANAGER (`HOTFIX-T089A`, closed); `GET /leave/:id` (single-record) and `GET /off-site` list access remain org-wide | Future: scope `GET /leave/:id` and `GET /off-site` list access by department if needed |
