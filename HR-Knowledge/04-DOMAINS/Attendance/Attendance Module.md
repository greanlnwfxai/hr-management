...
| PATCH | /attendance/risk-reviews/:id/approve | ✅ | SUPER_ADMIN, HR_ADMIN | Convenience: set status APPROVED |
| PATCH | /attendance/risk-reviews/:id/reject | ✅ | SUPER_ADMIN, HR_ADMIN | Convenience: set status REJECTED |
...
**Status (`v1.2.89`, REQ-002F): CLOSED.** See [[Off-site Work Mode]] §Admin Web Off-site
Review UI for the RBAC/privacy rationale and [[ADR-033 Off-site Review Access Scope and CI Throttle Policy]]
for why MANAGER access and the no-raw-GPS display were kept as-is.
...
- Follow-up: **SEC-ATT-001 Cross-Platform Attendance Anti-Spoofing** — this
  ...
  Android build, spec §15 Open Question #3),
  [docs/SEC_ATT_006A_IOS_APP_ATTEST_DEVICECHECK_FEASIBILITY.md](../../../docs/SEC_ATT_006A_IOS_APP_ATTEST_DEVICECHECK_FEASIBILITY.md)
  (with [docs/CTO_SUMMARY_SEC_ATT_006A.md](../../../docs/CTO_SUMMARY_SEC_ATT_006A.md))
  for the **SEC-ATT-006A** iOS App Attest / DeviceCheck feasibility &
  architecture decision — which **also recommends DEFER** (App Attest/DeviceCheck
  are native iOS framework APIs with no WebKit entry point; require an approved
  native iOS build, the same shared native-build decision),
  [docs/CTO_SUMMARY_SEC_ATT_007A.md](../../../docs/CTO_SUMMARY_SEC_ATT_007A.md)
  for **SEC-ATT-007A** — the attendance risk scoring + review queue **backend
  foundation** (complete): a new `AttendanceRiskReview` table records a
...
- **Device/app integrity (iOS App Attest / DeviceCheck) is likewise unavailable on the current PWA** — SEC-ATT-006A assessed it and **recommends DEFER**: App Attest (`DCAppAttestService`) and DeviceCheck (`DCDevice`) are native iOS framework APIs bound to an Apple app identity + Secure Enclave with no WebKit entry point, so they cannot run on the PWA-only STEP Connect surface and report `deviceIntegritySignal: UNAVAILABLE`. App Attest is the app/device-integrity attestation (with a stateful, per-key monotonic assertion counter the backend must persist to verify — unlike Android's stateless Play Integrity); DeviceCheck is a secondary per-device abuse-bit store, not an integrity attestation. Implementation (SEC-ATT-006) is blocked until a native iOS build strategy is approved — the **same shared native-build decision** as SEC-ATT-005 (spec §15 Open Question #3). See [docs/SEC_ATT_006A_IOS_APP_ATTEST_DEVICECHECK_FEASIBILITY.md](../../../docs/SEC_ATT_006A_IOS_APP_ATTEST_DEVICECHECK_FEASIBILITY.md)
- **Off-site review `reviewedBy` name not resolved** — `GET /attendance/offsite-review`'s `REVIEW_SELECT` projection returns `reviewedById` (UUID) only; the Admin Web page can't show a reviewer name (unlike the analogous risk-reviews page, which does join `reviewedBy`). Fix is a `select`-shape-only change (relation already exists in `schema.prisma`), not implemented — see `docs/CTO_SUMMARY_REQ_002F_OFFSITE_ADMIN_REVIEW_UI.md` §Known Limitations
- **SEC-ATT-007A risk-review rows have no Admin Web UI yet** (SEC-ATT-007B) — the queue is API-only today, browsable via `GET /attendance/risk-reviews`. `attendanceId` is null on essentially every automatically-created row: the payload/nonce checks that trigger a row run before the attendance record is created or fetched, so there is nothing to link to yet — rows are still traceable via `employeeId`/`userId`/`createdAt`. No time-window deduplication: a client that repeatedly omits the nonce or `capturedAt` during rollout produces one row per attempt, not a rolled-up counter — HR is expected to bulk-triage/`IGNORE` known bulk categories rather than review 1:1. `DEVICE_INTEGRITY_UNAVAILABLE`/`NATIVE_ATTESTATION_UNAVAILABLE` reason codes exist in the schema but are **not** wired to any current hook — recording them on literally every PWA clock action would be 100% noise; they're reserved for a future SEC-ATT-005/006 native signal. See [docs/CTO_SUMMARY_SEC_ATT_007A.md](../../../docs/CTO_SUMMARY_SEC_ATT_007A.md)
